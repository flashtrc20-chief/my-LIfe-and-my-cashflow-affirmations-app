...
#   * the main class name
#   * -classpath
#   * -D...=... to select a proxy for the JVM
#   * and three other properties as set previously
#   * the application (main) class
#
# Determine the Java command to use to start the JVM.
find_java_version() {
    local java_version=$("$JAVACMD" -version 2>&1 | grep -E 'version "[0-9]')
    echo "$java_version"
}
...
GRADLE_OPTS="$GRADLE_OPTS -XX:+IgnoreUnrecognizedVMOptions --add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.util=ALL-UNNAMED"
